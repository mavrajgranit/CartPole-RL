...
def replay():
    ...
    nq = target_network(next_state)
    targets = outputs.detach().clone()
    for i in range(batch_size):
        targets[i][actions[i]] = rewards[i] + nts[i]*nq[i][actions[i]]*discount_factor
...
plt.plot(lossmeans)
plt.title("Mean Loss")
#plt.savefig('./Plots/CartPole-MDQ/meanloss.png',bbox_inches='tight')
plt.figure(1)
plt.plot(rewardmeans,color="orange")
plt.title("Mean Reward")
#plt.savefig('./Plots/CartPole-MDQ/meanreward.png',bbox_inches='tight')
plt.figure(2)
plt.plot(qvalues,color="red")
plt.title("Mean Q-Value")
#plt.savefig('./Plots/CartPole-MDQ/meanqvalue.png',bbox_inches='tight')
plt.show()