...
for e in range(epochs):
        ...
        while True:
            ...
            target = qs.clone()
            nq = target_network(preprocess(new_state))
            target[action] = nt * discount_factor * nq[action] + reward
            l += learn(qs, target)
...
plt.plot(lossmeans)
plt.title("Mean Loss")
plt.savefig('./Plots/CartPole-DQ/meanloss.png',bbox_inches='tight')
plt.figure(1)
plt.plot(rewardmeans,color="orange")
plt.title("Mean Reward")
plt.savefig('./Plots/CartPole-DQ/meanreward.png',bbox_inches='tight')
plt.figure(2)
plt.plot(qvalues,color="red")
plt.title("Mean Q-Value")
plt.savefig('./Plots/CartPole-DQ/meanqvalue.png',bbox_inches='tight')
plt.show()